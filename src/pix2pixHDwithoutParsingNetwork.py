...
class Pix2PixHD_mask_Model(BaseModel):
    ...
    def init_loss_filter(self, use_gan_feat_loss, use_vgg_loss, use_l2_loss):
        ...
        def loss_filter(kl_loss,l2_mask_image,g_gan, g_gan_feat, g_vgg, d_real, d_fake, l2_image, g2_gan,d2_real,d2_fake):
            return [l for (l,f) in zip((kl_loss,l2_mask_image,g_gan,g_gan_feat,g_vgg,d_real,d_fake,l2_image,g2_gan,d2_real,d2_fake),flags) if f]
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.loss_names = self.loss_filter('KL_embed','L2_mask_image','G_GAN','G_GAN_Feat','G_VGG','D_real','D_fake','L2_image','G2_GAN','D2_real','D2_fake')
    ...
    def encode_input(self, label_map, inst_map=None, real_image=None, feat_map=None, image_affine=None, infer=False):             
        if self.opt.label_nc == 0:  #self.opt.label_nc =11
            input_label = label_map.data.cuda()
        else:
            # create one-hot vector for label map 
            size = label_map.size()    #size = ([4, 1, 256, 256])
            #print('label_map',label_map.size())tmux 
            oneHot_size = (size[0], self.opt.label_nc, size[2], size[3]) #oneHot_size =(4, 11, 256, 256) 
            input_label = torch.cuda.FloatTensor(torch.Size(oneHot_size)).zero_() #zeros of dim oneHot_size
            input_label = input_label.scatter_(1, label_map.data.long().cuda(), 1.0) #Enter scattered random variables in random places for input_label
        ...
        input_label = Variable(input_label, volatile=infer)   #input_label = ([4, 11, 256, 256])

        # real images for training
        if real_image is not None:
            real_image = Variable(real_image.data.cuda())  #real_image=([4, 3, 256, 256])
    ...
    def forward(self, bg_image, label, inst, image, feat, image_affine, mask_list, ori_label, infer=False, type="sample_net"):
        if type == "sample_net":
            ...
            #bg_image: ('no. of train images', 3,256,256)
            #label:('no. of train images', 1,256,256)
            #inst: (4)
            #image: ('no. of train images', 3,256,256)
            #feat: (4)
            #image_affine: ('no. of train images', 3,256,256)
            #mask_list:(4,6)
            # ori_label:('no. of train images', 1,256,256)
    ...
    def forward_sample_net(self, label, inst, image, feat, image_affine, infer=False):
        ...
        return self.loss_filter( zero_tensor,zero_tensor,loss_G_sample_GAN,loss_D2_fake,loss_D2_real,zero_tensor,zero_tensor,zero_tensor,zero_tensor,zero_tensor,zero_tensor,loss_l1_label, loss_l1_vector,zero_tensor,loss_MFM ),None if not infer else fake_image
    ...
    def forward_vae_net(self, bg_image, label, inst, image, feat, image_affine, mask_list, ori_label, infer=False):
        ...
        # start
        #label : ([BATCHSIZE, 1, 256, 256])
        #inst = 0, feat =0
        #mask_list : List consistig of 6 elements, which are center of the features in mask, 2 for each feature viz, R-eye, L-eye and mouth 
        #ori_label and  image_affine not important
        #print('real_image',real_image.shape)   #([BATCHSIZE, 3, 256, 256])
        #print('input_label',input_label.shape)  #([BATCHSIZE, label_nc, 256, 256]) , label_nc=11
        #print('inst_map',inst_map.shape)        #([BATCHSIZE])
        #print('real_bg_image',real_bg_image.shape) #([BATCHSIZE, 3, 256, 256])
        #print('label.size()',label.size())         #([BATCHSIZE, 1, 256, 256])
        '''mask_skin_pic = label[0,:,:,:].cpu()
        mask_skin_pic= transforms.functional.to_pil_image(mask_skin_pic)   
        mask_skin_pic.save('label.png')'''
    
        #These next 4 lines are empty boxes to house the generate image of individual features
        mask4_image = torch.zeros(label.size()[0],3,32,48).cuda()  #Mask_4 is for Left eye and 3,32,48 is general image for the eye
        #print('mask4_image',mask4_image.shape)    #([BATCHSIZE, 3, 32, 48])
        mask5_image = torch.zeros(label.size()[0],3,32,48).cuda()    #Mask_5 is for right Eye
        mask_mouth_image = torch.zeros(label.size()[0],3,80,144).cuda()
        mask_mouth = torch.zeros(label.size()[0],3,80,144).cuda() #3,80,144 is general image for the mouth

        #By Panday Mask for left and right eye
        left_eye_mask=(label==4).type(torch.cuda.FloatTensor)  #Get Left eye mask
        LeftEye_mask=mask4_image.clone()
        right_eye_mask=(label==5).type(torch.cuda.FloatTensor)  #Get Left eye mask
        RightEye_mask = mask5_image.clone()

        '''mask_skin_pic = left_eye_mask[0,:,:,:].cpu()
        mask_skin_pic= transforms.functional.to_pil_image(mask_skin_pic)   
        mask_skin_pic.save('LeftEye.png')'''

        mask_skin = ((label==1)+(label==2)+(label==3)+(label==6)).type(torch.cuda.FloatTensor)
        '''mask_skin_pic = mask_skin[0,:,:,:].cpu()
        mask_skin_pic= transforms.functional.to_pil_image(mask_skin_pic)   
        mask_skin_pic.save('mask_skin_pic.png')'''
        #print('mask_skin', mask_skin.shape)
        mask_skin_image = mask_skin * real_image  #For Source image picking up entire skin except both eyes, hair and mouth

        mask_hair = (label==10).type(torch.cuda.FloatTensor)
        mask_hair_image = mask_hair * real_image #Picking up only hair image

        mask_mouth_whole = ((label==7)+(label==8)+(label==9)).type(torch.cuda.FloatTensor) #Picking up only mouth image

        for batch_index in range(0,label.size()[0]):  #label.size()[0]=[4] which is BATCH SIZE
            ...
            LeftEye_mask[batch_index] = left_eye_mask[batch_index,:,int(mask_list[batch_index][0])-16:int(mask_list[batch_index][0])+16,int(mask_list[batch_index][1])-24:int(mask_list[batch_index][1])+24]
            ...
            RightEye_mask[batch_index] = right_eye_mask[batch_index,:,int(mask_list[batch_index][2])-16:int(mask_list[batch_index][2])+16,int(mask_list[batch_index][3])-24:int(mask_list[batch_index][3])+24]
        ...
        '''mask_skin_pic = RightEye_mask[0,:,:,:].cpu()
        mask_skin_pic= transforms.functional.to_pil_image(mask_skin_pic)   
        mask_skin_pic.save('RightEye_mask.png)'''
        # use masked mouth region
        #mask_mouth is the mask of the mouth
        #5 Mask tensor Names: LeftEye_mask, RightEye_mask, mask_mouth, mask_skin, mask_hair
        mask_mouth_image = mask_mouth * mask_mouth_image    #([4, 3, 80, 144])
        '''mask_mouth_image=mask_mouth_image[0,:,:,:]#torch.squeeze(mask_mouth_image,-1)
        print(mask_mouth_image.size()) #([4, 3, 80, 144])
        trans = transforms.ToPILImage()
        imggg= trans(mask_mouth_image)
        imggg.save('outputs/mask_mouth_image_Final.png')'''

        #label_encoder is used to encode the label(Em) of the Mask-Guided Generative Sub-Network
        encode_label_feature = self.netG.forward(input_label,type="label_encoder")   #([BATCHSIZE, 256, 64, 64])
        #bg_encoder is used to train the encoder(Eb) of 'Background Fusing Sub-Network'
        bg_feature = self.netG.forward(real_bg_image,type="bg_encoder")   #([BATCHSIZE, 64, 256, 256]) 
        #print('bg_feature',bg_feature.shape)
        mask_bg = (label==0).type(torch.cuda.FloatTensor)   #([4, 1, 256, 256])
        #print('mask_bg',mask_bg.shape)
        mask_bg_feature = mask_bg * bg_feature    #([BATCHSIZE, 64, 256, 256])
        '''mask_skin_pic = mask_bg[0,:,:,:].cpu()
        mask_skin_pic= transforms.functional.to_pil_image(mask_skin_pic)   
        mask_skin_pic.save('mask_bg.png')'''
        #print('mask_bg_feature',mask_bg_feature.shape)
        ...
        correct_sample4 = random_sample4 * variances4 + mus4  #([BATCHSIZE, 512]) Encodded, embedded layer for left eye
        #print('correct_sample4',correct_sample4.shape)
        ...
        correct_sample5 = random_sample5 * variances5 + mus5   #([BATCHSIZE, 512])  Encodded, embedded layer for left eye
        ...
        #print('mask_skin_image',mask_skin_image.shape)
        ...
        #print('mus_skin',mus_skin.shape)
        correct_sample_skin = random_sample_skin * variances_skin + mus_skin   #([BATCHSIZE, 512])  Encodded, embedded layer for skin
        #print('correct_sample_skin',correct_sample_skin.shape)
        ...
        #print('mask_skin', mask_skin.shape)
        #print('reconstruce_mask_skin_image', reconstruce_mask_skin_image.shape)
        ...
        correct_sample_hair = random_sample_hair * variances_hair + mus_hair #([BATCHSIZE, 512])  Encodded, embedded layer for hair
        #print('correct_sample_hair',correct_sample_hair.shape)
        ...
        correct_sample_mouth = random_sample_mouth * variances_mouth + mus_mouth  #([BATCHSIZE, 512])  Encodded, embedded layer for mouth
        #print('correct_sample_mouth',correct_sample_mouth.shape)
        ...
        left_eye_tensor = torch.zeros(encode_label_feature.size()).cuda()   #([BATCHSIZE, 256, 64, 64])
...
# insert unpaired part
        reorder_left_eye_tensor = torch.zeros(encode_label_feature.size()).cuda() #([4, 256, 64, 64])
        #print('reorder_left_eye_tensor', reorder_left_eye_tensor.shape)
        reorder_right_eye_tensor = torch.zeros(encode_label_feature.size()).cuda() #([4, 256, 64, 64])
        reorder_mouth_tensor = torch.zeros(encode_label_feature.size()).cuda() #([4, 256, 64, 64])


        new_order = torch.randperm(label.size()[0])   # torch.Size([4])
        #print('new_order',new_order.shape)

        #print('decode_embed_feature4', decode_embed_feature4.shape)   #([4, 256, 8, 12])
        reorder_decode_embed_feature4 = decode_embed_feature4[new_order]  #([4, 256, 8, 12])
        #print('reorder_decode_embed_feature4',reorder_decode_embed_feature4.shape)
        ...
        #Just verifying the mean locations of image masks just like it is done here:: mask4_image[batch_index] = real_image[batch_index....
        for batch_index in range(0,label.size()[0]):
            try:
                ...
                print("wrong0 ! ")
        #print('reorder_left_eye_tensor',reorder_left_eye_tensor.shape)
        #print('reorder_right_eye_tensor',reorder_right_eye_tensor.shape)
        #print('reorder_mouth_tensor',reorder_mouth_tensor.shape)
        #print('torch.cat((encode_label_feature,left_eye_tensor,right_eye_tensor,decode_embed_feature_skin,decode_embed_feature_hair,mouth_tensor)',torch.cat((encode_label_feature,left_eye_tensor,right_eye_tensor,decode_embed_feature_skin,decode_embed_feature_hair,mouth_tensor),1).shape)
        ...
        #bg_decoder is nothing but the final decoder(Gb) inside Background Fusing Sub-Network
...
########################################################################################################
        ...
        return self.loss_filter( loss_KL,loss_mask_image,loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_D_real, loss_D_fake, loss_L2_image, loss_G2_GAN, loss_D2_real, loss_D2_fake), None if not infer else reconstruct_image, None if not infer else reconstruce_mask4_image, None if not infer else reconstruce_mask5_image, None if not infer else reconstruce_mask_skin_image, None if not infer else reconstruce_mask_hair_image, None if not infer else reconstruce_mask_mouth_image, None if not infer else reconstruct_transfer_image
